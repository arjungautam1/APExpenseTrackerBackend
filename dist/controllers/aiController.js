...
exports.extractBulkTransactions = exports.autoCategorize = exports.scanBill = void 0;
...
Example for "salary deposit":
{
  ...
  "confidence": "high"
        ...
        const matchedCategory = categories.find(cat => cat.name.toLowerCase() === (parsed.category_name || '').toLowerCase());
        if (!matchedCategory) {
            console.log('Category not found, using fallback');
            const fallbackCategory = categories.find(cat => cat.name.toLowerCase().includes('other') &&
                cat.type === (parsed.transaction_type || 'expense')) || categories[0];
...
// @access  Private
const extractBulkTransactions = async (req, res) => {
    try {
    ...
    - For merchant names, use just the business name without store numbers or locations`;
        ...
        const processedTransactions = await Promise.all((parsed.transactions || []).map(async (transaction, index) => {
            console.log(`Processing transaction ${index + 1}:`, transaction);
            const matchedCategory = categories.find(cat => cat.name.toLowerCase() === (transaction.category_name || '').toLowerCase());
...
exports.extractBulkTransactions = extractBulkTransactions;
